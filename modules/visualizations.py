...
def create_future_assets_chart(future_assets_result: Dict[str, Any], current_age: int = None) -> go.Figure:
    """
    미래 자산 추정 차트 생성
    ...
    if use_age_axis:
        ...
        if yearly_breakdown and 'age' in yearly_breakdown[0]:
            ...
            ages = [current_age] + [current_age + item['year'] for item in yearly_breakdown]
        assets = [future_assets_result.get('current_assets', 0)] + [item['assets'] for item in yearly_breakdown]
        x_data = ages
        x_title = "나이 (세)"
    ...
    fig.update_layout(
        ...
        height=400,
        showlegend=use_age_axis and yearly_breakdown and 'is_retired' in yearly_breakdown[0]
    )
...
def create_risk_score_chart(risk_result: Dict[str, Any]) -> go.Figure:
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            'axis': {'range': [None, 100]},
            'bar': {'color': "darkred" if total_score >= 75 else 
                   "orange" if total_score >= 50 else
                   "yellow" if total_score >= 25 else "green"},
            'steps': [
...
def create_risk_breakdown_chart(risk_result: Dict[str, Any]) -> go.Figure:
    ...
    fig = go.Figure(data=[
        go.Bar(
            x=labels,
            y=values,
            marker_color=['#d62728' if v > 20 else '#ff7f0e' if v > 10 else '#2ca02c' for v in values],
            text=values,
            textposition='auto',
        )
    ])
...
def create_scenario_comparison_chart(comparison_result: Dict[str, Any]) -> go.Figure:
    ...
    if base_breakdown:
        ...
        assets = [base_scenario.get('current_assets', 0)] + [item['assets'] for item in base_breakdown]
    ...
    for i, scenario_data in enumerate(scenarios):
        ...
        if yearly_breakdown:
            years = [0] + [item['year'] for item in yearly_breakdown]
            assets = [scenario_data.get('current_assets', 0)] + [item['assets'] for item in yearly_breakdown]
...
def create_retirement_goal_chart(
    ...
    Returns:
        go.Figure: Plotly 그래프 객체
    """
    from modules.calculations import find_required_return_rate, find_optimal_contribution_rate
    
    # 여러 저축 금액에 대한 필요한 수익률 계산
    contribution_scenarios = list(range(50, 501, 50))  # 50만원부터 500만원까지 50만원 단위
    required_rates = []
    achievable_flags = []
    
    for contrib in contribution_scenarios:
        required_rate, result = find_required_return_rate(inputs, contrib, withdrawal_rate)
    ...
    achievable_contributions = [contrib for contrib, achievable in zip(contribution_scenarios, achievable_flags) if achievable]
    achievable_rates = [rate for rate, achievable in zip(required_rates, achievable_flags) if achievable]
    unachievable_contributions = [contrib for contrib, achievable in zip(contribution_scenarios, achievable_flags) if not achievable]
    unachievable_rates = [rate for rate, achievable in zip(required_rates, achievable_flags) if not achievable]
    
    # 달성 가능한 영역
    if achievable_contributions:
        fig.add_trace(go.Scatter(
            x=achievable_contributions,
            y=achievable_rates,
            mode='lines+markers',
            name='목표 달성 가능',
    ...
    # 현재 선택한 값 표시
    current_required_rate, _ = find_required_return_rate(inputs, current_monthly_contribution, withdrawal_rate)
    current_required_rate = min(15.0, current_required_rate)
    
    fig.add_trace(go.Scatter(
        x=[current_monthly_contribution],
