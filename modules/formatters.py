...
def generate_future_assets_insight(future_assets_result: Dict[str, Any]) -> str:
    ...
    if future_assets > current_assets:
        growth = ((future_assets - current_assets) / current_assets * 100) if current_assets > 0 else 0
        return (f"{years}년 후 예상 자산은 {format_currency(future_assets)}입니다. "
...
def generate_retirement_insight(retirement_result: Dict[str, Any]) -> str:
    ...
    if is_sustainable:
        return (f"은퇴까지 {years_to_retirement}년 남았습니다. "
