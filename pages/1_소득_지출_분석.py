...
from modules.download import create_json_download, get_download_filename
...
if run_simulation:
    ...
    with col3:
        ...
        if "monthly_fixed_expense" in inputs and "monthly_variable_expense" in inputs:
            ...
            st.metric(
                "월간 변동비", format_currency(inputs["monthly_variable_expense"])
            )
            monthly_total = (
            ...
            st.metric(
                "연간 고정 지출", format_currency(inputs.get("annual_fixed_expense", 0))
            )
    ...
    if "inflation_rate" in inputs:
        st.info(
            f"📊 사용된 설정: 인플레이션율 {inputs.get('inflation_rate', 2.5):.1f}%"
        )
    ...
    with col1:
        st.metric(
            ...
            delta=format_currency(
                future_assets_result.get("future_assets", 0) - inputs["total_assets"]
            ),
        )

    with col2:
        st.metric("재정 건전성 등급", grade_result.get("grade", "D"))

    with col3:
        st.metric("월 저축 가능액", format_currency(monthly_savings))
    ...
    with col1:
        ...
        chart = create_future_assets_chart(
            future_assets_result, current_age=inputs.get("current_age")
        )
    ...
    with col1:
        st.metric(
            "소득 대비 지출 비율",
            format_percentage(grade_result.get("expense_ratio", 0)),
        )

    with col2:
        st.metric(
            "자산 대비 부채 비율", format_percentage(grade_result.get("debt_ratio", 0))
        )

    with col3:
        st.metric(
            "비상금 지속 가능 개월",
            f"{grade_result.get('emergency_fund_months', 0):.1f}개월",
        )

    with col4:
        st.metric(
            "월 저축 가능액", format_currency(grade_result.get("monthly_savings", 0))
        )
    ...
    **4% 현금화율 기준**: 은퇴 자산의 4%를 매년 인출하여 생활비로 사용하는 방식입니다.
    ...
    # 초기 필요한 수익률 계산
    initial_required_rate, initial_result = find_required_return_rate(
        inputs, default_monthly_contribution, 4.0
    )
    initial_required_rate = max(
        0.0, min(15.0, initial_required_rate)
    )  # 0-15% 범위로 제한

    # 인터랙티브 그래프 먼저 표시
    st.subheader("📈 저축 금액 vs 수익률 관계 그래프")
    st.markdown(
        "**💡 그래프에서 초록색 영역은 목표 달성이 가능한 조합입니다. 슬라이더를 움직여 추이를 확인하세요.**"
    )
    ...
    retirement_chart = create_retirement_goal_chart(
        inputs, monthly_contribution, annual_return_rate, 4.0
    )
    ...
    if success_goal and goal_result:
        ...
        with col_goal2:
            st.metric(
                ...
                delta=format_currency(
                    goal_result.get("projected_assets", 0)
                    - goal_result.get("target_assets", 0)
                ),
                delta_color=(
                    "normal" if goal_result.get("is_achievable", False) else "inverse"
                ),
