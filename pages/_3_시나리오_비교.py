...
with col1:
    scenario_input = st.text_input(
        "시나리오 1", value="지출 10% 감소", help="예: '지출 10% 감소', '연봉 5% 증가'"
    )
    ...
    try:
        ...
        if parsed:
            st.session_state.scenarios.append(
                {"name": scenario_input, "parsed": parsed}
            )
...
if not comparison_scenarios:
        ...
        comparison_data.append(
            {
                ...
                "자산 증가율 (%)": round(
                    (
                        (
                            (scenario["future_assets"] - inputs["total_assets"])
                            / inputs["total_assets"]
                            * 100
                        )
    ...
    with col1:
        if best_scenario:
            st.success(
                f"✅ **최적 시나리오**: {best_scenario.get('scenario_name', 'N/A')}"
            )
            st.metric(
                "미래 자산", format_currency(best_scenario.get("future_assets", 0))
            )

    with col2:
        if worst_scenario:
            st.warning(
                f"⚠️ **최악 시나리오**: {worst_scenario.get('scenario_name', 'N/A')}"
            )
            st.metric(
                "미래 자산", format_currency(worst_scenario.get("future_assets", 0))
            )
