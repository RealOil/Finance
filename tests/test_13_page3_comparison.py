...
class TestPage3Comparison(unittest.TestCase):
    """페이지 3 테스트"""
    
    def test_page_file_exists(self):
        """페이지 파일이 존재하는지 확인"""
        page_file = PROJECT_ROOT / "pages" / "3_시나리오_비교.py"
        self.assertTrue(page_file.exists(), "pages/3_시나리오_비교.py 파일이 존재해야 합니다")
    ...
    def test_page_structure(self):
        ...
        with open(page_file, 'r', encoding='utf-8') as f:
            ...
            self.assertIn('parse_scenario', content)
            self.assertIn('calculate_scenario', content)
            self.assertIn('compare_scenarios', content)
...
def run_all_tests():
    """모든 테스트 실행"""
    print("=" * 60)
    print("작업 1.13: 페이지 3 - 시나리오 비교 테스트 시작")
    print("=" * 60)
    
    # 테스트 스위트 생성
    loader = unittest.TestLoader()
    suite = loader.loadTestsFromTestCase(TestPage3Comparison)
